...
exports.generateMockRounds = (req, res) => {
    ...
    const MAX_PUTTS = 4; // matches putt picker options 0–4 in the UI
    const tees = [1, 2, 3, 4, 5];

    db.all(
        'SELECT hole_number, par FROM hole_defs WHERE course_id = 1 ORDER BY hole_number',
        [],
        (err, holeDefs) => {
            if (err) return res.status(500).json({ error: err.message });

            const parByHole = Object.fromEntries(holeDefs.map((h) => [h.hole_number, h.par]));
            let completed = 0;
            let seedError = null;
            ...
            const finishRound = () => {
                completed++;
                if (completed === count) {
                    if (seedError) return res.status(500).json({ error: seedError });
                    res.json({ message: `Generated ${count} rounds` });
                }
            };
            ...
            for (let i = 0; i < count; i++) {
                ...
                db.run(
                    `INSERT INTO rounds (user_id, course_id, tee_id, date, holes_played, total_score, status, completed_at)
                     VALUES (?, ?, ?, ?, ?, ?, 'completed', datetime('now'))`,
                    [userId, courseId, teeId, dateStr, holesPlayed, totalScore],
                    function (insertErr) {
                        if (insertErr) {
                            seedError = insertErr.message;
                            return finishRound();
                        }
                        ...
                        for (let h = 1; h <= holesPlayed; h++) {
                            ...
                            let strokes = par + getRandomInt(-1, 2);

                            if (h === holesPlayed) {
                                strokes = totalScore - currentScore;
                            }
                            if (strokes < 1) strokes = 1;

                            const maxPutts = Math.min(MAX_PUTTS, Math.max(0, strokes - 1));
                            const putts = getRandomInt(0, maxPutts);
                            const fairwayHit = par > 3 ? getRandomInt(0, 1) : null;

                            db.run(
                                `INSERT INTO hole_scores (round_id, hole_number, strokes, putts, par, fairway_hit)
                                 VALUES (?, ?, ?, ?, ?, ?)`,
                                [roundId, h, strokes, putts, par, fairwayHit],
                                (holeErr) => {
                                    if (holeErr) seedError = holeErr.message;
                                }
                            );
